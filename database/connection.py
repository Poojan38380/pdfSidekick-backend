...
async def get_connection() -> Optional[asyncpg.Pool]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            print_info(f"Successfully connected to database (attempt {attempt + 1}/{MAX_RETRIES})")
            return pool
        except Exception as e:
            print_error(f"Error connecting to the database (attempt {attempt + 1}/{MAX_RETRIES}): {e}")
